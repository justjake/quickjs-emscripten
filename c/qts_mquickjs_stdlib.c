/*
 * quickjs-emscripten custom stdlib for mquickjs
 *
 * Based on vendor/mquickjs/mqjs_stdlib.c
 * This file adds the host function trampoline needed for vm.newFunction()
 *
 * Build: cc -o qts_mquickjs_stdlib qts_mquickjs_stdlib.c
 * Generate: ./qts_mquickjs_stdlib > qts_mquickjs_stdlib.h
 */
#include <math.h>
#include <stdio.h>
#include <string.h>
...
/*
 * Note: The trampoline function (qts_host_trampoline) is just referenced by name
 * in js_c_function_decl below. It's defined in interface-mquickjs.c and will be
 * resolved at link time when building the WASM module.
 *
 * All the standard class definitions from mqjs_stdlib.c follow.
 */
 ...
 *
 * The function index for user closures starts at JS_CFUNCTION_USER (1).
 * We define qts_host_trampoline at index 1 so QTS_NewFunction can use it
 * to create host-callable functions.
 */
#define JS_CFUNCTION_qts_host_trampoline JS_CFUNCTION_USER

static const JSPropDef js_c_function_decl[] = {
    /* must come first if "bind" is defined - index 0 (JS_CFUNCTION_bound) */
    JS_CFUNC_SPECIAL_DEF("bound", 0, generic_params, js_function_bound ),
    /* Host function trampoline - index 1 (JS_CFUNCTION_USER) */
    JS_CFUNC_SPECIAL_DEF("__qts_host_trampoline__", 0, generic_params, qts_host_trampoline ),
    JS_PROP_END,
};

int main(int argc, char **argv)
{
    return build_atoms("qts_mquickjs_stdlib", js_global_object, js_c_function_decl, argc, argv);
}
