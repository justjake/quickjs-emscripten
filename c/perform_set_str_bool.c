 ...
 */
QTS_CommandStatus perform_set_str_bool(QTS_CommandEnv *env, JSValueSlot target_slot, uint8_t bool_val, SetPropFlags flags, char *key_ptr, uint32_t key_len) {
    JSValue target = OP_GET_JSVALUE(env, target_slot, "set_str_bool: target");

    JSAtom key_atom = JS_NewAtomLen(env->ctx, key_ptr, key_len);
    OP_ERROR_IF(env, key_atom == JS_ATOM_NULL, "set_str_bool: failed to create key atom");

    int ret = qts_set_or_define_prop_atom(env->ctx, target, key_atom, JS_NewBool(env->ctx, bool_val), flags);
    JS_FreeAtom(env->ctx, key_atom);
    OP_ERROR_IF(env, ret < 0, "set_str_bool: exception");
