 ...
 */
QTS_CommandStatus perform_set_str_int32(QTS_CommandEnv *env, JSValueSlot target_slot, SetPropFlags flags, char *key_ptr, uint32_t key_len, int32_t int_val) {
    ...
    int ret = qts_set_or_define_prop_atom(env->ctx, target, key_atom, JS_NewInt32(env->ctx, int_val), flags);
