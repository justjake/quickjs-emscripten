...
QTS_CommandStatus perform_get_value(QTS_CommandEnv *env, JSValueSlot result_slot, JSValueSlot source_slot, JSValueSlot key_slot) {
    JSValue source = OP_GET_JSVALUE(env, source_slot, "get_value: source value");
    JSValue key = OP_GET_JSVALUE(env, key_slot, "get_value: key value");

    JSAtom key_atom = JS_ValueToAtom(env->ctx, key);
    OP_ERROR_IF(env, key_atom == JS_ATOM_NULL, "get_value: failed to convert key to atom");

    JSValue result = JS_GetProperty(env->ctx, source, key_atom);
    JS_FreeAtom(env->ctx, key_atom);
    OP_SET_JSVALUE(env, result_slot, result);

    OP_ERROR_IF(env, JS_IsException(result), "get_value: threw an exception");
