 ...
 */
QTS_CommandStatus perform_slot_load(QTS_CommandEnv *env, AnySlot out_slot, SlotType out_slot_type, void *in_ptr) {
    OP_ERROR_IF(env, !in_ptr, "slot_load: in_ptr is null");

    uint32_t item_size = QTS_SlotTypeItemBytes(out_slot_type);
    OP_ERROR_IF(env, item_size == 0, "slot_load: unknown slot type");
