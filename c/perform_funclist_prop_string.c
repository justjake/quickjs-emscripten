...
QTS_CommandStatus perform_funclist_prop_string(QTS_CommandEnv *env, FuncListSlot list, JSPropFlags flags, uint8_t index, char *str_ptr, uint32_t str_len, char *name_ptr) {
    (void)str_len; // String is null-terminated, length not needed for JSCFunctionListEntry
    ...
    OP_ERROR_IF(env, funclist->entries == NULL, "funclist_prop_string: list not initialized");
