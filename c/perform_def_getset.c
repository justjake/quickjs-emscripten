...
QTS_CommandStatus perform_def_getset(QTS_CommandEnv *env, JSValueSlot target_slot, uint8_t maybe_key_len, JSPropFlags flags, HostRefId getter_ref, HostRefId setter_ref, char *key_ptr) {
    ...
    if (getter_ref != 0) {
        ...
        if (JS_IsException(getter_host_ref)) {
            ...
            OP_ERROR(env, "def_getset: failed to create getter host ref");
        }
        getter = JS_NewCFunctionData(env->ctx, (JSCFunctionData *)qts_funclist_getter, 0, getter_ref, 1, &getter_host_ref);
        if (JS_IsException(getter)) {
            JS_FreeAtom(env->ctx, key_atom);
            OP_ERROR(env, "def_getset: failed to create getter function");
        }
    }

    // Create setter function if setter_ref is provided
    ...
    if (setter_ref != 0) {
        ...
        if (JS_IsException(setter_host_ref)) {
            ...
            JS_FreeAtom(env->ctx, key_atom);
            OP_ERROR(env, "def_getset: failed to create setter host ref");
        }
        setter = JS_NewCFunctionData(env->ctx, (JSCFunctionData *)qts_funclist_setter, 1, setter_ref, 1, &setter_host_ref);
        if (JS_IsException(setter)) {
            JS_FreeValue(env->ctx, getter);
            JS_FreeAtom(env->ctx, key_atom);
            OP_ERROR(env, "def_getset: failed to create setter function");
        }
    }

    // Define the property with getter/setter
    // JS_DefinePropertyGetSet consumes getter and setter values
    int ret = JS_DefinePropertyGetSet(env->ctx, target, key_atom, getter, setter, flags | JS_PROP_HAS_GET | JS_PROP_HAS_SET);
    JS_FreeAtom(env->ctx, key_atom);
    OP_ERROR_IF(env, ret < 0, "def_getset: exception");
