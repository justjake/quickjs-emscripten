...
ifeq ($(QUICKJS_LIB),mquickjs)
	...
	# Include paths for the generated stdlib header and mquickjs private headers
	WRAPPER_DEFINES+=-I$(BUILD_WRAPPER) -I$(QUICKJS_ROOT)
	INTERFACE_SOURCE=interface-mquickjs.c
	# mquickjs needs a generated stdlib header
	MQUICKJS_STDLIB_H=$(BUILD_WRAPPER)/qts_mquickjs_stdlib.h
...
# For mquickjs, this depends on the generated stdlib header
$(BUILD_WRAPPER)/interface.o: $(WRAPPER_ROOT)/$(INTERFACE_SOURCE) $(WASM_SYMBOLS) $(MQUICKJS_STDLIB_H) | $(EMCC_SRC)
	$(MKDIRP)
...
###############################################################################
# mquickjs stdlib generation
# The stdlib is generated by compiling and running a C program that outputs
# the stdlib header to stdout. This header defines the JS global object,
# builtin classes, and function table.
#
# Uses host cc (not emcc) since this runs at build time to generate a header.

# Compile the stdlib generator (using host cc, not emcc)
# This is a native executable that runs at build time
$(BUILD_WRAPPER)/qts_mquickjs_stdlib_gen: $(WRAPPER_ROOT)/qts_mquickjs_stdlib.c $(QUICKJS_ROOT)/mquickjs_build.c
	$(MKDIRP)
	cc -o $@ -I$(QUICKJS_ROOT) $^

# Generate the stdlib header by running the generator
# Use -m32 since wasm32 is a 32-bit target
$(BUILD_WRAPPER)/qts_mquickjs_stdlib.h: $(BUILD_WRAPPER)/qts_mquickjs_stdlib_gen
	$< -m32 > $@
