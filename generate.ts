...
const TS_FFI_INTERFACE_NAME = ASYNCIFY ? "QuickJSAsyncFFI" : "QuickJSFFI"
const TS_FFI_CLASS_NAME = (() => {
  const asyncPart = ASYNCIFY ? "Async" : "Sync"
  const debugPart = DEBUG ? "Debug" : "Release"
  return `QuickJS${asyncPart}${debugPart}FFI`
})()
...
function main() {
  ...
  if (command === "ffi-interface") {
    writeFile(destination, buildFFIInterface(matches))
    return
  }

  if (command === "types") {
    writeFile(destination, buildTypes())
    return
  }
...
interface FunctionDecl {
  typescriptFunctionName: string
  typescriptFunctionType: string
  cwrapImplementation: string
}

function getTypescriptFunction(args: {
  functionName: string
...
function buildFIIDecls(matches: RegExpExecArray[]) {
  const parsed = getAvailableDefinitions(matches)
  const decls: FunctionDecl[] = []
  parsed.forEach((fn) => {
    if (!fn.returnType.attributes.has("AsyncifyOnly") || ASYNCIFY) {
      decls.push(getTypescriptFunction({ ...fn, async: false }))
    }

    if (fn.returnType.attributes.has("MaybeAsync") && ASYNCIFY) {
      decls.push(getTypescriptFunction({ ...fn, async: true }))
    }
  })
  return decls
}

function buildFFIInterface(matches: RegExpExecArray[]) {
  const decls = buildFIIDecls(matches)
  ...
  const methods = decls.map(
    (decl) => `  ${decl.typescriptFunctionName}: ${decl.typescriptFunctionType}`
  )
  const interfaceString = `
// This file generated by "generate.ts ffi" in the root of the repo.
import type { ${importFromFfiTypes.join(", ")} } from "../types-ffi"
 ...
 */
export interface ${TS_FFI_INTERFACE_NAME} {
  readonly DEBUG: boolean
${methods.join("\n")}
}
  `
  return interfaceString.trim()
}
...
function buildFFI(matches: RegExpExecArray[]) {
  ...
  const ffiInterfaceFile = ASYNCIFY ? "interface.ASYNCIFY" : "interface.SYNC"
  const moduleTypeName = ASYNCIFY ? "QuickJSAsyncEmscriptenModule" : "QuickJSEmscriptenModule"

  const funcs = decls.map(
    ({ cwrapImplementation, typescriptFunctionName, typescriptFunctionType }) =>
      `  ${typescriptFunctionName}: ${typescriptFunctionType} =\n    ${cwrapImplementation}`
  )
  const classString = `
// This file generated by "generate.ts ffi" in the root of the repo.
import { ${TS_FFI_INTERFACE_NAME} as Interface } from "./${ffiInterfaceFile}"
 ...
 * @private
 */
export default class ${TS_FFI_CLASS_NAME} {
...
// build a .d.ts file that exports some generated types for typescript.
function buildTypes() {
  const incomingModuleJsApis: string[] = require("./incomingModuleJsApis.json")
  let ts: string[] = [
    `// This file generated by "generate.ts types" in the root of the repo.`,
    "export type EnabledIncomingModuleJsApis = ",
  ]
  for (const apiName of incomingModuleJsApis) {
    ts.push(`  | '${apiName}'`)
  }
  return ts.join("\n") + "\n"
}
